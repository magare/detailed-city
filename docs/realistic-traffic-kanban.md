...
| RTV-D01 | GLM 5.1 execution workflow | `docs/realistic-traffic-glm-workflow.md` documents the development-time GLM 5.1 dispatch workflow, context packet, provider and credential policy, retry/fallback behavior, review gates, board responsibilities, and explicit prohibition on runtime language-model calls; verification: `npm run build`, `npm run test:e2e`, `git diff --check`. |
| RTV-101 | GLM 5.1 card execution prompt | `docs/realistic-traffic-glm-card-prompt.md` adds a reusable one-card GLM 5.1 prompt template with ticket, dependency, main-board linkage, git state, dirty-file, architecture, file-scope, verification, browser-check, output, blocker, and rewrite sections; verification: `npm run build`, `npm run test:e2e`, `git diff --check`. |
...
| RTV-204 | Bus vehicle contract | `src/city/data-contracts/cityContracts.ts` adds `BusVehicleService` fields for transit route, ordered stops, next stop, dwell time, door side, schedule offset, headway group, passenger load estimate, and bus-lane permission; `src/city/data-contracts/validation/validateTrafficPlan.ts` validates bus-only service payloads and rejects missing route/stop references; `tests/e2e/city-traffic-routing.spec.ts` covers valid bus service fixtures, missing bus service, missing route refs, and missing stop refs; verification: `npm run build`, focused traffic routing e2e, `npm run test:e2e`, `npm run test:e2e:browser`, desktop/mobile browser checks, runtime AI scan, `git diff --check`. |
| RTV-502 | Deterministic simulation tests | `tests/e2e/city-traffic-simulation.spec.ts` adds browser-free deterministic trace coverage proving repeated seeded simulation ticks produce identical position, speed, behavior, stop-timer, stopping, and wrap-around snapshots; existing focused tests cover acceleration, braking, stop timers, wrap-around, and seeded runtime state behavior; verification: focused simulation e2e. |
