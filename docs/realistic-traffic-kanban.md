...
| ID | Priority | Size | Card | Details | Acceptance | Dependencies |
| --- | --- | --- | --- | --- | --- | --- |
| RTV-201 | P0 | M | Vehicle type taxonomy | Add explicit vehicle classes such as car, taxi, van, bus, delivery truck, heavy truck, service vehicle, emergency vehicle, and parked vehicle. Include allowed modes, dimensions, capacity hints, visual variant tags, and default asset binding IDs. | Contracts and generated vehicles expose `vehicleClass`, physical dimensions, asset binding, and behavior profile. Existing traffic validation still passes. | None |
| RTV-202 | P0 | M | Vehicle dynamics contract | Add deterministic motion parameters: max speed, preferred speed, acceleration, braking, comfortable deceleration, minimum following distance, reaction time, turn speed, and stop tolerance. | Generated traffic vehicles carry finite realistic dynamics values. Validator rejects negative or impossible values. | RTV-201 |
| RTV-203 | P0 | M | Vehicle runtime state contract | Separate static vehicle plan data from mutable runtime state: route offset, current speed, target speed, behavior state, stop timer, lane-change state, signal state, and replay seed. | Runtime state can be initialized from a traffic plan without mutating generated domain contracts. | RTV-202 |
...
| ID | Priority | Size | Card | Details | Acceptance | Dependencies |
| --- | --- | --- | --- | --- | --- | --- |
| RTV-401 | P0 | M | Traffic simulation system shell | Move per-vehicle update behavior out of `src/world/city/City.ts` into a dedicated traffic simulation system. Keep rendering handoff separate from simulation state. | `City.update()` delegates to a traffic simulation object. Existing vehicle movement still works. Build and e2e smoke tests pass. | RTV-203 |
| RTV-402 | P0 | M | Acceleration and braking | Replace constant speed stepping with deterministic acceleration toward target speed and braking toward stop zones. | Vehicles ease into motion, brake before stops, never exceed speed limit, and preserve existing stop behavior. Unit tests cover accelerate, cruise, brake, and wrap-around. | RTV-401 |
