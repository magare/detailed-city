...
| KAN-312 | P1 | M | City object grouping and query API | Add grouping semantics for districts, corridors, slices, scenario layers, render batches, validation selections, and authored object sets. | Debug tools can query by group, owner domain, slice, scenario, and render layer without scanning raw arrays. | KAN-101 |

### Blueprint

| ID | Priority | Size | Card | Details | Acceptance | Dependencies |
| --- | --- | --- | --- | --- | --- | --- |
| KAN-321 | P1 | M | Master plan model | Represent city form, center hierarchy, skyline intent, protected open space, water edge intent, and growth boundaries as executable blueprint data. | District generation reads master-plan constraints; diagnostics show center/open-space/growth-boundary counts. | KAN-302 |
| KAN-322 | P1 | M | District character and transition rules | Expand district rules with use mix, landmark targets, density gradients, transition buffers, material/style hints, and allowed street profiles. | District boundaries, heights, uses, and style hints come from blueprint; validator catches illegal district transitions. | KAN-321 |
...
| KAN-324 | P1 | M | Constraint layer | Model setbacks, protected corridors, easements, clearances, no-build zones, hazard buffers, view corridors, waterfront buffers, and emergency access corridors. | Parcels/buildings/roads validate against constraints; conflicts are visible in overlay. | KAN-321 |
| KAN-325 | P1 | M | Resilience goals | Encode redundancy, climate adaptation, evacuation routes, emergency access, continuity goals, shelters, and recovery priorities. | Emergency and flood cards can consume resilience targets; validation reports coverage gaps. | KAN-324 |
| KAN-326 | P1 | M | City metrics model | Track walkability, density, open-space access, service coverage, traffic, energy, emissions, and quality checks. | Metrics are computed after generation and exposed in diagnostics/debug panel. | KAN-322, KAN-325 |

### Land

| ID | Priority | Size | Card | Details | Acceptance | Dependencies |
| --- | --- | --- | --- | --- | --- | --- |
| KAN-341 | P1 | M | Administrative boundaries | Add city limits, wards, neighborhoods, service areas, ownership zones, and jurisdiction overlays. | Blocks/parcels belong to boundaries; overlay can show neighborhoods/wards; validator catches orphan blocks. | KAN-321 |
| KAN-342 | P1 | M | Block model expansion | Add alleys, internal access, permeability metrics, buildable envelopes, frontage classes, and block subdivision constraints. | Blocks expose internal access and permeability; parcels read block envelopes. | KAN-341 |
| KAN-343 | P1 | M | Parcel model expansion | Add setbacks, lot splits, development rights, frontage priority, parcel-level constraints, and parcel fit helpers. | Buildings use parcel envelopes rather than raw centered rectangles; validator checks setbacks and frontage. | KAN-342, KAN-324 |
| KAN-344 | P1 | M | Zoning model | Add allowed uses, height controls, FAR, coverage, overlays, buffers, frontage rules, density, and form-based rules. | Parcel/building generation reads zoning; validator reports use, height, FAR, coverage, and buffer violations. | KAN-343 |
...
| KAN-347 | P1 | M | Waterways expansion | Add canals/channels/culverts/docks/outfalls and water edge continuity, not just one rectangular river. | Waterways have edge segments and crossing/outfall references; validator catches disconnected water edges. | KAN-341 |
