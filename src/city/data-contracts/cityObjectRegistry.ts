...
const WIND_COMFORT_ZONE_KIND = String.raw`(?:bridge-effect|downdraft-risk|public-space-comfort|sheltered-area|waterfront-exposure|wind-corridor)`;
...
export const CITY_OBJECT_KIND_REGISTRY_ENTRIES = [
  ...
  entry(
    'curb-activation',
    ['curb-activation-<activation-kind>-<curb-zone-id>'],
    [exact(String.raw`curb-activation-${CURB_ACTIVATION_KIND}-curb-zone-${ROAD_ID}-${SIDEWALK_SIDE}-segment-\d+-${CURB_USE}`)],
    required(['curb-zone'])
  ),
  entry(
    'culture-anchor',
  ...
  entry(
    'education-anchor',
    ['education-anchor-<education-anchor-kind>'],
    [exact(String.raw`education-anchor-${EDUCATION_ANCHOR_KIND}`)],
    required(['civic-anchor'])
  ),
  ...
  entry('industrial-facility', ['industrial-facility-<building-id>'], [exact(String.raw`industrial-facility-${BUILDING_ID}`)], required(['economy-anchor', 'building'])),
  ...
  entry(
    'wind-comfort-zone',
    ['wind-comfort-<zone-kind>-<index>'],
    [exact(String.raw`wind-comfort-${WIND_COMFORT_ZONE_KIND}-\d+`)],
    required(['building', 'park', 'plaza-zone', 'road-segment', 'waterfront-open-space'])
  ),
  entry(
    'sidewalk-graph-edge',
