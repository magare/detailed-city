...
test('debug panel exposes current city diagnostics and can collapse', async ({ page }) => {
  ...
  expect(panelText).toContain('17806/17806 tagged');
  ...
  expect(panelText).toContain('6 formats, 16849 objects');
  expect(panelText).toContain('Registry');
  expect(panelText).toContain('70 kinds');
