...
test('debug panel exposes current city diagnostics and can collapse', async ({ page }) => {
  ...
  expect(panelText).toContain('19922/19922 tagged');
  ...
  expect(panelText).toContain('6 formats, 18965 objects');
  expect(panelText).toContain('Registry');
  expect(panelText).toContain('82 kinds');
