...
test('scene layer diagnostics are deterministic and match current city data', () => {
  ...
  expect(layers.find((layer) => layer.id === 'networks')?.objectCount).toBe(city.roads.length + traffic.markings.length);
  ...
  expect(layers.find((layer) => layer.id === 'overlays')?.objectCount).toBe(
    city.verticalSlices.length + city.cityMetrics.length + city.constraints.length + city.resilienceGoals.length
  );
});
